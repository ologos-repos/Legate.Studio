...
from .core import get_effective_tier, get_trial_status, login_required
...
def get_or_create_stripe_products() -> dict:
    ...
    for tier_key, product_config in STRIPE_PRODUCTS.items():
        try:
            ...
            existing = stripe.Product.search(query=f"metadata['legato_tier']:'{tier_key}'")
...
def get_or_create_customer(user_id: str) -> str:
    ...
    user = db.execute(
        "SELECT stripe_customer_id, github_login, email FROM users WHERE user_id = ?", (user_id,)
    ).fetchone()
...
def create_portal_session(user_id: str) -> str:
    ...
    session = stripe.billing_portal.Session.create(customer=customer_id, return_url=f"{base_url}/billing/")
...
def switch_subscription_tier(user_id: str, new_tier: str) -> dict:
    ...
    db = _get_db()
    user = db.execute("SELECT stripe_subscription_id, tier FROM users WHERE user_id = ?", (user_id,)).fetchone()
    ...
    return {"success": True, "message": f"Switched to {new_tier} plan", "new_tier": new_tier}
...
def reactivate_subscription(user_id: str) -> dict:
    ...
    """
    db = _get_db()
    user = db.execute("SELECT stripe_subscription_id FROM users WHERE user_id = ?", (user_id,)).fetchone()
...
def handle_checkout_completed(session_data: dict):
    ...
    user = db.execute("SELECT github_login, is_beta FROM users WHERE user_id = ?", (user_id,)).fetchone()
...
def handle_subscription_updated(subscription: dict):
    ...
    if items:
        ...
        try:
            ...
            config = db.execute("SELECT key FROM system_config WHERE value = ?", (price_id,)).fetchone()
...
def handle_subscription_deleted(subscription: dict):
    ...
    if user["is_beta"]:
        logger.info(
            f"Skipping subscription deletion downgrade for beta "
            f"user {user['github_login']} — beta tier is admin-managed"
        )
...
def handle_invoice_payment_failed(invoice: dict):
    ...
    user = db.execute("SELECT user_id, github_login FROM users WHERE stripe_customer_id = ?", (customer_id,)).fetchone()
