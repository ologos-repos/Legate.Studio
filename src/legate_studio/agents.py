        ...
        if agent.get("related_entry_id"):
            ...
            if legato_db:
                ...
                note = legato_db.execute(
                    "SELECT title FROM knowledge_entries WHERE entry_id = ?", (entry_id,)
                ).fetchone()
    ...
    try:
        ...
        for nid in note_ids:
            entry = legato_db.execute(
                ("SELECT entry_id, title, category, content, domain_tags FROM knowledge_entriesWHERE entry_id = ?"),
                (nid.strip(),),
            ).fetchone()
            if entry:
...
def api_queue_chord():
    ...
    try:
        ...
        # Check entry exists
        entry = legato_db.execute(
            "SELECT id, title, needs_chord FROM knowledge_entries WHERE entry_id = ?", (entry_id,)
        ).fetchone()
        ...
        return jsonify({"status": "success", "entry_id": entry_id, "needs_chord": True})
...
def api_queue_from_entry():
    ...
    try:
        ...
        entry = legato_db.execute("SELECT * FROM knowledge_entries WHERE entry_id = ?", (entry_id,)).fetchone()
...
def api_pending_count():
    ...
    try:
        ...
        result = db.execute(
            "SELECT COUNT(*) FROM agent_queue WHERE status = 'pending' AND user_id = ?", (user_id,)
        ).fetchone()
...
def api_approve_agent(queue_id: str):
    ...
    try:
        ...
        if user_id:
            ...
            logger.info(
                f"api_approve_agent: oauth_token present={bool(oauth_token)}, "
                f"len={len(oauth_token) if oauth_token else 0}"
            )

            if not oauth_token:
                return jsonify(
                    {
                        "error": ("GitHub authorization expired. Please re-authenticate to approveagents."),
                        "needs_reauth": True,
...
"""
        ...
        if spawn_success and related_entry_id:
            try:
                ...
                legato_db.commit()
                logger.info(
                    f"Updated {len(entry_ids)} Library entries with chord_status=active, chord_repo={chord_repo_full}"
                )
            except Exception as e:
    ...
    try:
        ...
        if user_id:
            ...
            if not oauth_token:
                return jsonify(
                    {
                        "error": ("GitHub authorization expired. Please re-authenticate to retryspawning."),
                        "needs_reauth": True,
                        "reauth_url": "/auth/github-app-login",
                    }
                ), 401
                                ...
                                if len(parts) >= 3:
                                    ...
                                    if new_frontmatter != frontmatter:
                                        ...
                                        commit_file(
                                            ...
                                            path=file_path,
                                            content=new_content,
                                            message="Reset chord fields: bulk rejection",
                                            token=token,
                                        )
...
def api_add_comment(queue_id: str):
    ...
    try:
        ...
        # Add new comment
        comments.append({"text": text, "author": username, "timestamp": datetime.now().isoformat() + "Z"})
        ...
        return jsonify({"status": "added", "comments": comments})
    ...
    try:
        ...
        if related_entry_id:
            ...
            for entry_id in entry_ids:
                ...
                try:
                    entry = legato_db.execute(
                        "SELECT file_path FROM knowledge_entries WHERE entry_id = ?", (entry_id,)
                    ).fetchone()
                            ...
                            if len(parts) >= 3:
                                ...
                                if new_frontmatter != frontmatter:
                                    ...
                                    commit_file(
                                        ...
                                        path=file_path,
                                        content=new_content,
                                        message="Reset chord fields: agent rejected",
                                        token=token,
                                    )
                                    logger.info(f"Updated frontmatter for {entry_id}: removed chord fields")
                except Exception as e:
...
def api_get_agent(queue_id: str):
    ...
    try:
        ...
        row = db.execute("SELECT * FROM agent_queue WHERE queue_id = ? AND user_id = ?", (queue_id, user_id)).fetchone()
...
def import_projects_from_routing(routing: list, run_id: int, db) -> dict:
    ...
    for item in routing:
        ...
        already_processed = db.execute(
            "SELECT id FROM sync_history WHERE run_id = ? AND item_id = ?", (run_id, item_id)
        ).fetchone()
...
### References
- Source entries: {", ".join(
    f"`{e['entry_id']}`" for e in group_entries
)}
...
def trigger_spawn_workflow(agent: dict, user_id: str = None) -> dict:
    ...
    if related_entry_id:
        try:
            ...
            entry = legato_db.execute(
                "SELECT title, content FROM knowledge_entries WHERE entry_id = ?", (entry_id,)
            ).fetchone()
            if entry:
