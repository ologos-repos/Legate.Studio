...
**Everything becomes a Note first.** All threads are classified as
KNOWLEDGE and stored in the Library. If a thread describes something
that needs implementation, it is flagged with `needs_chord: true`
for escalation.
...
EXTRACTOR_PROMPT = """You are extracting structured knowledge artifacts
from classified transcript threads for the LEGATO system.
...
class MotifProcessor:
    ...
    def _classify_threads(self, threads: list[dict]):
        ...
        if categories:
            category_desc = "\n".join(
                [f"   - {c['name'].upper()}: {c.get('description', c['display_name'])}" for c in categories]
            )
        ...
        threads_json = json.dumps([{"id": t["thread_id"], "content": t["content"]} for t in threads], indent=2)

        response = self._call_claude(
            system=system_prompt, user=f"Classify these threads:\n\n{threads_json}", api_key=api_key
        )
        ...
        try:
            ...
            raise ValueError(f"Invalid classification response: {e}") from e
    ...
    def _simple_correlate(self, thread: dict, user_db) -> tuple[str, str | None, float]:
...
Generate the complete markdown artifact with frontmatter."""
                ...
                markdown = self._call_claude(system=EXTRACTOR_PROMPT, user=prompt, api_key=api_key)
    ...
    def _queue_chord_entries(self):
        ...
        from .rag.database import get_user_db_path, init_agents_db
...
def process_motif_sync(transcript: str, user_id: str, source_id: str = None) -> dict:
    ...
    db.execute(
        ...
        (job_id, user_id, input_content, source_id, status, worker_id, locked_until, created_at, updated_at)
