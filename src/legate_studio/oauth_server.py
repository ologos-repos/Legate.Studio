...
def _get_or_create_user_id(github_id: int, github_login: str) -> str:
    ...
    row = db.execute("SELECT user_id, created_at FROM users WHERE github_id = ?", (github_id,)).fetchone()
...
def oauth_protected_resource():
    ...
    return jsonify({"resource": base, "authorization_servers": [base]})
...
def _auto_register_trusted_client(client_id: str, redirect_uri: str, client_name: str = None) -> bool:
    ...
    existing = db.execute("SELECT client_id FROM oauth_clients WHERE client_id = ?", (client_id,)).fetchone()
    ...
    try:
        ...
        db.commit()
        logger.info(
            f"Auto-registered trusted OAuth client: {client_id} ({client_name}) with redirect_uri: {redirect_uri}"
        )
        return True
...
def authorize():
    ...
    if not github_client_id:
        return jsonify({"error": "server_error", "error_description": "GitHub OAuth not configured"}), 500
...
def handle_mcp_github_callback():
    ...
    try:
        token_response = requests.post(
            GITHUB_TOKEN_URL, data=token_data, headers={"Accept": "application/json"}, timeout=10
        )
...
def _handle_authorization_code_grant():
    ...
    # Look up auth code
    db = get_db()
    auth_code = db.execute("SELECT * FROM oauth_auth_codes WHERE code = ?", (code,)).fetchone()
...
def require_mcp_auth(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if github_id:
            db = get_db()
            canonical = db.execute("SELECT user_id FROM users WHERE github_id = ?", (github_id,)).fetchone()
...
def cleanup_expired_oauth_sessions():
    ...
    try:
        ...
        expired_codes = db.execute("DELETE FROM oauth_auth_codes WHERE expires_at < ?", (now,)).rowcount

        # Clean up expired refresh tokens (30-day TTL)
        expired_sessions = db.execute("DELETE FROM oauth_sessions WHERE expires_at < ?", (now,)).rowcount

        if expired_codes > 0 or expired_sessions > 0:
            db.commit()
            logger.info(
                f"OAuth cleanup: removed {expired_codes} expired auth codes, {expired_sessions} expired sessions"
            )
        else:
            db.commit()
    except Exception as e:
