    ...
    return jsonify(
        {
            ...
            "note": ("These are the exact category definitions that will be sent to Conduct forclassification"),
        }
    )
...
def api_upload():
    ...
    if current_app.config.get("LEGATO_MODE") == "multi-tenant":
        return jsonify(
            {
                "error": ("Motif processing is temporarily disabled while we build secure multi-tenantsupport."),
                "disabled": True,
            }
        ), 503
    ...
    if success:
        return jsonify({"success": True, "message": message, "source_id": source_id})
    else:
...
def api_transcribe():
    ...
    if not allowed_audio_file(filename):
        return jsonify({"error": f"Invalid audio format. Supported: {', '.join(ALLOWED_AUDIO_EXTENSIONS)}"}), 400

    # Read audio data
    audio_data = audio_file.read()

    if len(audio_data) > MAX_AUDIO_SIZE:
        return jsonify({"error": f"Audio file too large. Maximum size is {MAX_AUDIO_SIZE // (1024 * 1024)}MB."}), 400
