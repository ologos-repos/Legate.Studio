...
def submit_job():
    ...
    if process_sync:
        ...
        return jsonify({"job_id": job_id, "status": "pending", "message": "Job queued for processing"})
...
def get_job_status(job_id: str):
    ...
    if job_dict.get("result_entry_ids"):
        job_dict["result_entry_ids"] = [eid.strip() for eid in job_dict["result_entry_ids"].split(",") if eid.strip()]
    else:
