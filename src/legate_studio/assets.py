...
from .core import get_user_library_repo, library_required
...
def update_asset(asset_id: str):
    ...
    try:
        ...
        # Check asset exists
        existing = db.execute("SELECT asset_id FROM library_assets WHERE asset_id = ?", (asset_id,)).fetchone()
        ...
        db.execute(f"UPDATE library_assets SET {', '.join(updates)} WHERE asset_id = ?", params)
        db.commit()
...
def get_markdown_ref(asset_id: str) -> str | None:
